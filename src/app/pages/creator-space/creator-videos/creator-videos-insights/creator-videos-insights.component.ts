...
export class CreatorVideosInsightsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.totalViews = _(this.videos).map(video => video.views).flatten().map((view: any) => view.viewCounter).value().reduce((a, b) => a + b, 0);
    this.totalWatchTime = _(this.videos).map(video => video.views).flatten().map((view: any) => view.viewsTimestamps).flatten().map(($timestamp: any) => $timestamp.duration).value().reduce((a, b) => a + b, 0);
  }
